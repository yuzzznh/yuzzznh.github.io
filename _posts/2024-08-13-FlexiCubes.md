...
- Figure 7에서 볼 수 있듯, <span style='color: #f4acb6'></span>$\textcolor{#f4acb6}{s(x)}$<span style='color: #f4acb6'>의 </span><span style='color: #f4acb6'>cube corners에서의 부호</span>가 conectivity와 adjacency 관계를 결정한다. MC에서 그랬던 것처럼 edge 상에서의 interpolation을 통해 $u_e$ 점들을 얻는 것으로 시작한다.
...
### 4.2 Flexible Dual Vertex Positioning & <span style='color: #f4acb6'>Interpolation Weights</span> <span style='color: #f4acb6'></span>$\textcolor{#f4acb6}{\alpha}$<span style='color: #f4acb6'>, </span>$\textcolor{#f4acb6}{\beta}$<span style='color: #f4acb6'></span>
...
- <span style='color: #f4acb6'>DMC에 additional flexibility를 도입</span>하기 위해, 저자들은 먼저 <span style='color: #f4acb6'>각 grid cell마다 </span>$\textcolor{#f4acb6}{\alpha \in \mathbb{R}^8_{>0}}$<span style='color: #f4acb6'>라는 weight 집합을 정의한다. </span>$\textcolor{#f4acb6}{\alpha}$<span style='color: #f4acb6'>는 </span><span style='color: #f4acb6'>각 cube corner에 양의 스칼라 값을 하나씩 연결짓는다. </span>따라서 $\alpha$는 각 edge 위에 있는 <span style='color: #f4acb6'>crossing point</span> <span style='color: #f4acb6'></span>$\textcolor{#f4acb6}{c_e}$<span style='color: #f4acb6'></span><span style='color: #f4acb6'>의 위치를 조정한다.</span> 그러면 <span style='color: #f4acb6'></span>$\textcolor{#f4acb6}{u_e}$<span style='color: #f4acb6'></span><span style='color: #f4acb6'>의 위치</span>를 결정하던 Equation 3은 다음의 Equation 5로 대체된다.
...
- 한편, 위와 마찬가지로, <span style='color: #a6a6a6'>original DMC에서처럼 dual vertex를 그냥 naive하게 primal face의 centroid에 위치시키기보다는,</span> 저자들은 <span style='color: #f4acb6'>각 grid cell마다 </span>$\textcolor{#f4acb6}{\beta \in \mathbb{R}^{12}_{>0}}$<span style='color: #f4acb6'>라는 weight 집합을 정의한다. </span>$\textcolor{#f4acb6}{\beta}$<span style='color: #f4acb6'>는 각 cube edge에 양의 스칼라 값을 하나씩 연결짓는다. (</span>$\textcolor{#f4acb6}{\alpha}$<span style='color: #f4acb6'>는 cube corner에, </span>$\textcolor{#f4acb6}{\beta}$<span style='color: #f4acb6'>는 cube edge에!) </span>따라서 <span style='color: #f4acb6'></span>$\textcolor{#f4acb6}{\beta}$<span style='color: #f4acb6'>는 </span><span style='color: #f4acb6'>각 face 안에서 dual vertex의 위치를 조정</span>하는 역할을 한다. 그러면 $\textcolor{#f4acb6}{v_d}$<span style='color: #f4acb6'>의 위치</span>를 결정하던 Equation 4는 다음의 Equation 6으로 대체된다.
...
### 4.3 Flexible Quad Splitting & <span style='color: #f4acb6'>Splitting Weights</span> <span style='color: #f4acb6'></span>$\textcolor{#f4acb6}{\gamma}$<span style='color: #f4acb6'></span>
...
- 우리는 <span style='color: #f4acb6'>각 grid cell마다 weight </span>$\textcolor{#f4acb6}{\gamma \in \mathbb{R}_{>0}}$<span style='color: #f4acb6'>를 정의</span>하며, $\gamma$는 (추출된 mesh에 포함되는) <span style='color: #f4acb6'>생성된 vertex들(cell마다 1~4개)로 propagate된다.</span> optimization-time "한정으로", mesh의 각 사각형 면은 하나의 midpoint vertex $\overline{v_d}$를 insert함으로써 4개의 삼각형으로 쪼개진다. (Figure 8 참고) 이때 이 midpoint의 위치는 다음의 Equation 7에 따라 결정되며, notation은 Figure 8에서와 동일하다.
$$\textcolor{#f4acb6}{\textcolor{#f4acb6}{ \overline{v_d} = \frac{\gamma_{c_1} \gamma_{c_3} \left( v_d^{c_1} + v_d^{c_3} \right) / 2 + \gamma_{c_2} \gamma_{c_4} \left( v_d^{c_2} + v_d^{c_4} \right) / 2}{\gamma_{c_1} \gamma_{c_3} + \gamma_{c_2} \gamma_{c_4}} }}$$
- 이는 면의 두 가지 가능한 대각선의 중간점들의 weighted combination이며, weights는 해당 vertex에 대한 𝛾 매개변수에서 가져온다. 직관적으로, 𝛾 가중치를 조정하면 두 가지 가능한 분할에서 발생하는 geometry들의 사이를 smoothly interpolate하게 된다. 𝛾 값을 optimize하면 objective of interest에 fit하는 split을 선택하는 데 도움이 된다. Optimization이 완료된 후, <span style='color: #f4acb6'>final extraction 시에는, midpoint vertex</span> $\textcolor{#f4acb6}{\overline{v_d}}$<span style='color: #f4acb6'>를 삽입하지 "않고", 단순히 𝛾 값의 곱이 더 큰 대각선을 따라 각 사각형을 분할한다.</span>
### 4.4 Flexible Grid Deformation & <span style='color: #f4acb6'>Deformation Vectors</span> <span style='color: #f4acb6'></span>$\textcolor{#f4acb6}{\delta}$<span style='color: #f4acb6'></span>
...
- 저자들은 GET3D의 3D 생성기에서 마지막 레이어만 수정하여 FlexiCubes의 각 cube에 대해 21개의 가중치를 추가로 생성하도록 했다. ($\alpha$, $\beta$ and $\gamma$)
